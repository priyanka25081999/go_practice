...
func AddNewEntryToFile(username string) {
	...
	if check.Size() != 0 {
		...
		for i, val := range arr {
			fmt.Println("*****len of value : ", len(val))
			if len(val) <= 1 {
				continue
			}

			splitName := strings.Split(arr[i], "/")
			fmt.Println("****splitted data 0:", splitName[0])

			uname = splitName[0]
			splitUname := strings.Split(uname, "---")
			fmt.Println("****SplitUname : ", splitUname[0])
			uname = splitUname[0]
	...
	for key, value := range userMap {
		...
		fmt.Println("*****uname :", uname, " key : ", key, " str:", str)
...
func List() {
	...
	for i = 1; i < len(splitName)-1; i += 2 {
		...
		//fmt.Println("IMP: ", uname, splitName[i], splitName[i+1])
